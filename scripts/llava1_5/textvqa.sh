...
TEXTVQADIR="$DATA_ROOT/textvqa"
ANSWER_DIR="$RESULT_DIR/answers"
mkdir -p "$ANSWER_DIR"

python -m llava.eval.model_vqa_loader \
    --model-path "$MODEL_PATH" \
    --question-file "$TEXTVQADIR/llava_textvqa_val_v051_ocr.jsonl" \
    --image-folder "$TEXTVQADIR/train_images" \
...
python scripts/check_textvqa_prompt_protocol.py \
    --question-file "$TEXTVQADIR/llava_textvqa_val_v051_ocr.jsonl" \
    --result-file "$ANSWER_DIR/$CKPT.jsonl"

python -m llava.eval.eval_textvqa \
    --annotation-file "$TEXTVQADIR/TextVQA_0.5.1_val.json" \
    --result-file "$ANSWER_DIR/$CKPT.jsonl"
