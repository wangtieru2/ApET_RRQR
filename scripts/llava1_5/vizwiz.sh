...
SPLIT="${VIZWIZ_SPLIT:-val}"
VIZWIZDIR="$DATA_ROOT/vizwiz"
...
python -m llava.eval.model_vqa_loader \
    ...
    --question-file "$VIZWIZDIR/llava_$SPLIT.jsonl" \
    --image-folder "$VIZWIZDIR/$SPLIT" \
...
python scripts/convert_vizwiz_for_submission.py \
    --annotation-file "$VIZWIZDIR/llava_$SPLIT.jsonl" \
    --result-file "$ANSWER_DIR/$CKPT.jsonl" \
    --result-upload-file "$UPLOAD_DIR/$CKPT.json"

if [[ "$SPLIT" == "val" ]]; then
    python scripts/eval_vqa_answers.py \
        --annotation-file "$VIZWIZDIR/VizWiz_val.json" \
        --result-file "$ANSWER_DIR/$CKPT.jsonl" \
